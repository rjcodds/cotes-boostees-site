...
async function postMonitoringDiff(env, prevBoosts, currentBoosts) {
	...
	for (const event of events) {
		try {
			...
			// un cycle complet (site instable, ou vraiment retirée puis réémise
			// avec un nouveau marketId), diffBoosts la voit comme "remove" puis,
			// plus tard, comme un "add" tout neuf -- plus aucun lien avec l'ancien
			// marketId à ce stade. Bug réel trouvé côté Unibet (mirroré ici par
			// prudence, même risque structurel) : "V.Gyokeres buteur et son
			// équipe gagne" et "J.Bellingham décisif..." repartis en double sur le
			// canal spawn à ~2h d'intervalle, marketId ET cote différents à chaque
			// fois. Mémoire indépendante du snapshot (`recentlyalerted:{contenu}`,
			// TTL SEEN_TTL_SECONDS = 6h). Distingue deux cas (demande explicite de
			// l'utilisatrice, ne jamais avaler un vrai changement en silence) :
