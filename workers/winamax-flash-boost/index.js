...
async function postMonitoringDiff(env, prevBoosts, currentBoosts) {
	...
	for (const event of events) {
		try {
			...
			// instable, ou vraiment retirée puis réémise avec un nouveau marketId
			// + cote ajustée), diffBoosts la voit comme "remove" puis, plus tard,
			// comme un "add" tout neuf -- plus aucun lien avec l'ancien marketId à
			// ce stade. Bug réel trouvé côté Unibet (mirroré ici par prudence,
			// même risque structurel) : "V.Gyokeres buteur et son équipe gagne" et
			// "J.Bellingham décisif..." repartis en double sur le canal spawn à 2h
			// d'intervalle, marketId ET cote différents à chaque fois -- même
			// contenu (event+marché), donc pas utile de re-alerter "Nouvelle
			// cote" une deuxième fois. Mémoire indépendante du snapshot, TTL
			// large (SEEN_TTL_SECONDS, 6h) pour couvrir ce genre de trou sans
			// supprimer une vraie nouvelle occurrence le lendemain.
			if (event.type === 'add') {
